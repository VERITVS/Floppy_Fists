...
export class Arc {
    ...
    set_articulation(theta, id = this.id) {
        ...
        if (this.is_dof.Rx) {
            ...
            if(theta[index]) this.dof.Rx = theta[index];
            else this.dof.Rx = 0;
            index += 1;
        }
        if (this.is_dof.Ry) {
        ...
        if (this.is_dof.Rz) {
            ...
            if(theta[index]) this.dof.Rz = theta[index];
            else this.dof.Rz = 0;
            index += 1;
        }
...
export class End_Effector extends Arc {
    ...
    solve_ik(target) {
        ...
        // Alternate Method if OUt of Reach
        if (target_dist > max_reach) {
            target = this.handle_out_of_range(target, root_pos, max_reach);
        }

        for (let i = 0; i < this.max_iterations; i++) {
            let end_pos = joints[joints.length - 1].get_global_position();
            if (end_pos.distanceTo(target) < this.threshold) break;

            // Iterate from the end effector to the base
    ...
    rotate_joint_towards_target(joint, target, max_reach) {
        ...
        // Ensure rotation respects degrees of freedom
